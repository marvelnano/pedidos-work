...
function initResponsables() {
  $('#btn-rotar-responsables').onclick = () => {
    const personasActivas = Storage.personas.all().filter(p => p.activo);
    // Excluir personas vaciadas en la rotación actual
    const rot = Storage.rotaciones.current();
    let excluidos = [];
    if (rot && rot.asignaciones) {
      rot.asignaciones.forEach(a => {
        if (a.personaId === null) {
          // Si el día está vaciado, excluir la persona que estaba antes
          // Pero como personaId es null, simplemente no se incluye
        }
      });
      // Excluir personas que ya están vaciadas
      const vaciados = rot.asignaciones.filter(a => a.personaId === null).map(a => a.dia);
      // Excluir personas que fueron vaciadas en la semana actual
      excluidos = rot.asignaciones.filter(a => a.personaId === null).map(a => a.personaId);
    }
    // Filtrar personas activas que no están vaciadas
    const personasParaRotar = personasActivas.filter(p => !excluidos.includes(p.id));
    Storage.rotaciones.rotateWeek(personasParaRotar);
    renderRotacion();
  };
...
function initDataActions() {
  ...
  if (!btnExport || !btnImport || !fileInput || !btnSaveFS || !btnOpenFS || !btnLoadPreset) {
    return;
  }
